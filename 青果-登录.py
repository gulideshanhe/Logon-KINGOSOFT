...
class KINGOSOFT():
    ...
    def sign(self,DES_KEY: str = "93015171955021563496877", SIGN_TIME: str = "2024-07-02 17:17:17") -> bool:
        ...
        if "操作成功!" in response.text:
            logging.debug("登录cookie:", self.session.cookies["JSESSIONID"])
            return True
        elif "账号或密码有误!" in response.text:
            logging.error("账号或密码有误!")
            return False
        else:
    ...
    def get_grade(self) -> list:
        ...
        for tr in table[1:]:
            td = tr.select("td")
            try:
                class_grade.append((td[1].text, td[12].text, td[8].text, td[9].text, td[10].text,))
            except IndexError:
                class_grade.append((td[1].text, td[8].text, "NULL", "NULL", "NULL",))
...
if __name__ == "__main__":
    kingosoft = KINGOSOFT("username", "password", "https://example")
    if kingosoft.sign():
